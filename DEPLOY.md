...
9. **Every subcategory Overview has a Learn More link.** The Overview panel (`category.html`) renders its "Learn more" link from the subcategory-level `google_guidance` field. An empty `google_guidance` drops the link from that subcategory's whole Overview page. This is NOT permitted unless the topic's docs are genuinely paywalled — and if any check on the page carries a public Learn More link, they are not paywalled, so the Overview has no excuse. Set `google_guidance` to the most authoritative verified doc among that subcategory's own checks' `learn_more` citations. QC check (must print nothing): `python3 -c "import json,glob;[print('MISSING:',f) for f in sorted(glob.glob('json/<platform>/**/*.json',recursive=True)) if not json.load(open(f)).get('google_guidance')]"`. The Magnite build shipped 25 of 27 Overviews empty before this invariant existed.
